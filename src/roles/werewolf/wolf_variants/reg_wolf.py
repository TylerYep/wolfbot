...
def get_wolf_statements(player_obj: Any, knowledge_base: KnowledgeBase) -> Tuple[Statement, ...]:
    ...
    if Role.VILLAGER in const.ROLE_SET and should_include_role(counts_dict, Role.VILLAGER):
    ...
    if Role.INSOMNIAC in const.ROLE_SET and should_include_role(counts_dict, Role.INSOMNIAC):
    ...
    if Role.TROUBLEMAKER in const.ROLE_SET and should_include_role(counts_dict, Role.TROUBLEMAKER):
    ...
    if Role.SEER in const.ROLE_SET and should_include_role(counts_dict, Role.SEER):
        for i, stated_role in enumerate(stated_roles):
            ...
            if stated_role not in (Role.NONE, Role.SEER) and i not in player_obj.wolf_indices:
