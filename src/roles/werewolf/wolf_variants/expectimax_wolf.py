...
def get_statement_expectimax(eval_fn, player_index, prev_statements, possible_statements, wolf_indices=[]):
    ...
    def expectimax(statement_list, state, ind, depth=const.EXPECTIMAX_DEPTH):
        ...
        if ind == player_index:         # Choose your own move, maximize val
            vals = _get_next_vals(statement_list, possible_statements, state, ind, depth, True)
            best_indices = [index for index, value in enumerate(vals) if value == max(vals)]
            best_move = possible_statements[random.choice(best_indices)]
